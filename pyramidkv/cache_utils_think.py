...
class DynamicCache(Cache):
    ...
    def __init__(self) -> None:
        ...
        self.mask = []
        self.avg_scores = []  # Used in `adathink` to keep track of average scores for each layer   
        self.queries = []  # Used in `adathink` to keep track of queries for each layer
    ...
    def update_flash(
        ...
        return self.key_cache[layer_idx], self.value_cache[layer_idx], self.key_cache_pruned[layer_idx], self.mask[layer_idx], self.avg_scores[layer_idx], self.queries[layer_idx]

    def update_flash_think(
        self,
        key_states: torch.Tensor,
        value_states: torch.Tensor,
        layer_idx: int,
        ...
        return self.key_cache[layer_idx], self.value_cache[layer_idx], self.key_cache_pruned[layer_idx], self.mask[layer_idx]
    ...
    def update_adathink(
        ...
        # Update the number of seen tokens
        # if layer_idx == 0:
        #     self._seen_tokens += key_states_pruned.shape[-2]
    ...
    def get_seq_length(self, layer_idx: Optional[int] = 0) -> int:
        """Returns the sequence length of the cached states. A layer index can be optionally passed."""
        # TODO: deprecate this function in favor of `cache_position`
        if len(self.key_cache) <= layer_idx and len(self.key_cache_pruned) <= layer_idx:
            return 0
        elif len(self.key_cache[layer_idx]) == 0:
            # If the pruned key cache is empty, we return the length of the full key cache
            return self.key_cache_pruned[layer_idx].shape[-2]
        else:
    ...
    def to_legacy_cache(self, cache_type="think") -> Tuple[Tuple[torch.Tensor], Tuple[torch.Tensor]]:
        ...
        for layer_idx in range(len(self)):
            ...
            elif cache_type == "adathink":
                # if len(self.key_cache) == 0:
                #     # If the key cache is empty, we return an empty tuple for the key states
                #     legacy_cache += ((self.key_cache_pruned[layer_idx], None, self.mask[layer_idx], self.value_cache[layer_idx], self.avg_scores[layer_idx], self.queries[layer_idx]),)
                # else:
                legacy_cache += ((self.key_cache_pruned[layer_idx], self.key_cache[layer_idx], self.mask[layer_idx], self.value_cache[layer_idx], self.avg_scores[layer_idx], self.queries[layer_idx]),)

        return legacy_cache

    @classmethod
    def from_legacy_cache(cls, past_key_values: Optional[Tuple[Tuple[torch.FloatTensor]]] = None, cache_type="think") -> "DynamicCache":
        ...
        if past_key_values is not None:
            for layer_idx in range(len(past_key_values)):
                if cache_type == "think":
                    ...
                    key_states_pruned, key_states, mask, value_states, avg_scores, queries = past_key_values[layer_idx]
                    cache.update_adathink(key_states_pruned, key_states, mask, value_states, avg_scores, queries, layer_idx)
        return cache
