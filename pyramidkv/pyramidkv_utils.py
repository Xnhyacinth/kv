...
def key_pruner_query_driven_adaptive(keys, q_states, recent_size=128, ratio=0.3):
    ...
    q_norm = torch.sqrt(torch.sum(queries ** 2, dim=-2, keepdim=True))
    del queries
    # k_norm = torch.pow(keys, 2)
    k_norm = keys ** 2
    contributions = k_norm * q_norm
    del k_norm
    sorted_indices = torch.argsort(contributions, dim=-1, descending=True)

    # group_indicator = torch.zeros(bsz, num_heads, seq_len).to(queries.device)
    # topk_ratios = [ratio]
    topk = head_dim - int(ratio * head_dim)
    mask = torch.zeros_like(keys, dtype=torch.bool).to(keys.device)
    mask.scatter_(-1, sorted_indices[..., :topk], 1)

    threshold = torch.topk(contributions, k=topk, dim=-1, sorted=False)[0][..., -1, None]  # Shape: (bsz, num_heads, seqlen, 1)
    # mask = contributions >= threshold  # Shape: (bsz, num_heads, seqlen, head_dim)
    
    # Compute average scores for low-contribution dimensions
    masked_scores = torch.where(mask, torch.tensor(0.0, device=keys.device), contributions)
    mask_sum = (~mask).sum(dim=-1, keepdim=True).clamp(min=1)  # Shape: (bsz, num_heads, seqlen, 1)
    avg_scores = masked_scores.sum(dim=-1, keepdim=True) / mask_sum
    # try:
    # threshold = torch.topk(contributions, k=topk, dim=-1)[0][..., -1, None]
    # mask_low = contributions < threshold 
    # masked_scores = torch.where(mask_low, contributions, torch.tensor(0.0, device=contributions.device)) 
    # mask_sum = mask_low.sum(dim=-1, keepdim=True)
    # mask_sum = mask_sum + (mask_sum == 0) * 1
    # avg_scores = (masked_scores.sum(dim=-1, keepdim=True) / mask_sum)
    # breakpoint()
    # print(f"avg_scores: {avg_scores.shape}")
    # except:
    # avg_scores = torch.tensor(0.0, device=keys.device)
    #     breakpoint()
    # return keys[:, :, :seqlen - recent_size, :][~mask[:, :, :seqlen - recent_size, :]].reshape(1, -1, seqlen - recent_size, topk), kv_states[:, :, seqlen - recent_size:, :], torch.where(mask == 1, torch.where(keys > 0, torch.tensor(1, dtype=keys.dtype), torch.tensor(-1, dtype=keys.dtype)), torch.tensor(0, dtype=keys.dtype)), avg_scores
    # return keys[~mask].reshape(1, -1, seqlen, topk), torch.where(mask == 1, torch.where(keys > 0, torch.tensor(1, dtype=keys.dtype), torch.tensor(-1, dtype=keys.dtype)), torch.tensor(0, dtype=keys.dtype)), avg_scores, q_norm
    # breakpoint()
    # return keys[mask].view(1, -1, seqlen, topk), mask.to_sparse(), avg_scores, q_norm
    # mask = mask.cpu().numpy()
    # mask = np.packbits(mask.cpu().numpy(), axis=-1)
    # return keys[mask].view(1, -1, seqlen, topk), np.packbits(mask.cpu().numpy(), axis=-1), avg_scores, q_norm
    # fromDlpack(to_dlpack(mask))
...
class SnapKVCluster():
    ...
    def update_think(self, key_states, query_states, value_states, attention_mask, num_key_value_groups):
        ...
        print(f"Think max_capacity_prompt {self.max_capacity_prompt}")
    ...
    def update_adathink(self, key_states, query_states, value_states, attention_mask, num_key_value_groups):
        ...
        if q_len < self.max_capacity_prompt:
            ...
            key_states = torch.cat([k_past_compress, k_cur], dim = 2)
            value_states = torch.cat([v_past_compress, v_cur], dim = 2)
            kv_pruned, mask, avg_scores, queries = key_pruner_query_driven_adaptive(key_states, query_states, self.recent_size, self.ratio)
            return kv_pruned, mask, avg_scores, value_states, queries
