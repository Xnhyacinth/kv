...
import torch.nn.functional as F
import warnings
from pyramidkv.cache_utils_think import Cache, DynamicCache, StaticCache
...
from pyramidkv.pyramidkv_utils import init_pyramidkv,init_snapkv,init_CAM,init_H2O,init_StreamingLLM,init_l2norm, init_adakv, init_headkv, init_think
import math
...
def recover_keys(key_pruned, queries, mask, avg_scores):
    ...
    # avg_scores = avg_scores.repeat(1, 1, 1, head_dim)
    
    avg_scores = avg_scores.expand(bsz, num_heads, seq_len, head_dim)
    new_values = torch.sqrt(avg_scores / queries)

    # recovered_keys = torch.zeros_like(key_pruned)
    recovered_keys = torch.zeros(bsz, num_heads, seq_len, head_dim, device=key_pruned.device, dtype=key_pruned.dtype)

    # Create a mask for positions where mask is 0
    # zero_mask = (mask == 0)  # Positions where mask is 0
    # pos_mask = (mask == 1)   # Positions where mask is 1
    # neg_mask = (mask == -1)  # Positions where mask is -1
    ...
    del mask, pos_mask, zero_indices, new_values, key_pruned
    # breakpoint()
    
    # Fill positions where mask == -1 with -new_values
    # recovered_keys[neg_mask] = -new_values[neg_mask]

    return recovered_keys


def llama_attn_forward_SnapKV_AdaThinK(
    self,
    ...
    **kwargs,
    ...
    if past_key_value is not None:
        ...
        if key_states.shape[-2] == kv_seq_len:
            ...
            kv_pruned, mask, avg_scores, value_states_compress, queries = self.kv_cluster.update_adathink(key_states, query_states, value_states, attention_mask, self.num_key_value_groups)
            past_key_value.update_adathink(kv_pruned, torch.empty(0, dtype=key_states.dtype).to(key_states.device), mask, value_states_compress, avg_scores, queries, self.layer_idx, cache_kwargs)
            # past_key_value.update_adathink(kv_pruned, None, mask, value_states_compress, avg_scores, queries, self.layer_idx, cache_kwargs)
        else:
    ...
    if query_states.shape[-2] == 1:
        ...
        mask = past_key_value.mask[self.layer_idx]
        queries = past_key_value.queries[self.layer_idx]
        # recover keys
        new_key_states = torch.cat([recover_keys(key_pruned, queries, mask, past_key_value.avg_scores[self.layer_idx]), key_states], dim=-2)
        attn_weights = torch.matmul(query_states, new_key_states.transpose(2, 3)) / math.sqrt(self.head_dim)
    ...
    **kwargs,
    ...
    if past_key_value is not None:
        ...
        if key_states.shape[-2] == kv_seq_len:
            ...
            self.kv_seq_len += q_len
            key_states, value_states, key_pruned, mask, avg_scores, queries = past_key_value.update_flash(key_states, value_states, self.layer_idx, cache_kwargs)
            key_states =  torch.cat([recover_keys(key_pruned, queries, mask, past_key_value.avg_scores[self.layer_idx]), key_states], dim=-2)
        past_key_value._seen_tokens=self.kv_seq_len
...
def llama_flash_attn2_forward_SnapKV_ThinK(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
    ...
    **kwargs,
    ...
    if past_key_value is not None:
        ...
        if key_states.shape[-2] == kv_seq_len:
            ...
            self.kv_seq_len += q_len
            key_states, value_states, key_pruned, mask = past_key_value.update_flash_think(key_states, value_states, self.layer_idx, cache_kwargs)
...
def adathink_model_forward(
    ...
    if (
        ...
        past_key_values = DynamicCache.from_legacy_cache(past_key_values, "adathink")
        logger.warning_once(
            "We detected that you are passing `past_key_values` as a tuple and this is deprecated and will be removed in v4.43. "
            "Please use an appropriate `Cache` class (https://huggingface.co/docs/transformers/v4.41.3/en/internal/generation_utils#transformers.Cache)"
        )
    ...
    next_cache = next_decoder_cache if use_cache else None
    if return_legacy_cache:
        next_cache = next_cache.to_legacy_cache('adathink')
...
def think_model_forward(
    ...
    if (
        ...
        return_legacy_cache = True
        past_key_values = DynamicCache.from_legacy_cache(past_key_values, "think")
        logger.warning_once(
            "We detected that you are passing `past_key_values` as a tuple and this is deprecated and will be removed in v4.43. "
            "Please use an appropriate `Cache` class (https://huggingface.co/docs/transformers/v4.41.3/en/internal/generation_utils#transformers.Cache)"
        )
